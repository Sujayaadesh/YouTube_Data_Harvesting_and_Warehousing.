...
# -------------- Queries that read data in the Mysql and display those data in streamlit(table_formate)
    ...
    if st.button(":orange[Average duration :clock9: of all videos in each channel and their corresponding channel names]"):
        query9= "select avg(video_duration) as Averade_duration , title as channel_name from video_details s1 join channels1 s2 join playlist1 s3 on s1.playlist_id = s3.playlist_id and s3.channel_id = s2.channel_id group by title"
    ...
    if st.button(":green[videos having highest number of comments and their corresponding channel names]"):
        query10 = "select commentCount,Video_name,title as Channel_name from video_details s1 join channels1 s2 join playlist1 s3 on s1.playlist_id = s3.playlist_id and s3.channel_id = s2.channel_id ORDER BY commentCount DESC limit 10"
